...
names(algae_asci_flow_lag1)
data_lag1 <- dplyr::select(algae_asci_flow_lag1, 1:3, 5,6,14, 20, one_of(algae.metrics), 4,33:66) %>% 
...
dat_mmi_hyb <- data_lag1[,c(8,15:(ncol(data_lag1)-1))]
names(dat_mmi_hyb) 
...
#   htmltools::em('Relative Influence for Lag 1 Year MATCH')),
...
ggplot(data=gbm1_ri_top, aes(x=varnames,y=RI)) +
  ...
  coord_flip() +
  labs(title=paste0("Lag 1 Year: Top ",gbm1_topn," vars for Hybrid MMI"))

ggsave(filename = "figs/lag1/lag1_brt_mmi_hybrid_top_RI_barplot.png",width = 7, height = 7, units = "in", dpi = 300)

###################################################
# OxyRed.DO_30.richness Lag 1 Year BRT --------------------------------------------------------
names(data_lag1)
# this matches algae from same year against flow from same year
...
# select cols of interest
dat_OxyRed <- data_lag1[,c(9,15:(ncol(data_lag1)-1))]
names(dat_OxyRed) 
...
ggplot(data=gbm1_ri_top, aes(x=varnames,y=RI)) +
  geom_bar(position="dodge",stat="identity") +
  coord_flip() +
  labs(title=paste0("Lag 1 Year: Top ",gbm1_topn," vars for OxyRed 30"))

ggsave(filename = "figs/lag1/lag1_brt_OxyRed_top_RI_barplot.png",width = 7, height = 7, units = "in", dpi = 300)

###################################################
# prop.spp.BCG4 Lag 1 Year BRT --------------------------------------------------------
names(data_lag1)
# this matches algae from same year against flow from same year
...
# select cols of interest
dat_BCG4 <- data_lag1[,c(10,15:(ncol(data_lag1)-1))]
names(dat_BCG4) 
...
ggplot(data=gbm1_ri_top, aes(x=varnames,y=RI)) +
  geom_bar(position="dodge",stat="identity") +
  coord_flip() +
  labs(title=paste0("Lag 1 Year: Top ",gbm1_topn," vars for BCG4"))

ggsave(filename = "figs/lag1/lag1_brt_BCG4_top_RI_barplot.png",width = 7, height = 7, units = "in", dpi = 300)

###################################################
# Salinity.BF.richness Lag 1 Year BRT --------------------------------------------------------
names(data_lag1)
# this matches algae from same year against flow from same year
...
# select cols of interest
dat_Salinity <- data_lag1[,c(11,15:(ncol(data_lag1)-1))]
names(dat_Salinity) 
...
ggplot(data=gbm1_ri_top, aes(x=varnames,y=RI)) +
  geom_bar(position="dodge",stat="identity") +
  coord_flip() +
  labs(title=paste0("Lag 1 Year: Top ",gbm1_topn," vars for Salinity"))

ggsave(filename = "figs/lag1/lag1_brt_Salinity_top_RI_barplot.png",width = 7, height = 7, units = "in", dpi = 300)




###################################################
# prop.spp.IndicatorClass_DOC_high_raw Lag 1 Year BRT --------------------------------------------------------
names(data_lag1)
# this matches algae from same year against flow from same year
...
# select cols of interest
dat_doc <- data_lag1[,c(12,15:(ncol(data_lag1)-1))]
names(dat_doc) 
...
ggplot(data=gbm1_ri_top, aes(x=varnames,y=RI)) +
  geom_bar(position="dodge",stat="identity") +
  coord_flip() +
  labs(title=paste0("Lag 1 Year: Top ",gbm1_topn," vars for DOC"))

ggsave(filename = "figs/lag1/lag1_brt_DOC_top_RI_barplot.png",width = 7, height = 7, units = "in", dpi = 300)

###################################################
# MMI.sba Lag 1 Year BRT --------------------------------------------------------
names(data_lag1)
# this matches algae from same year against flow from same year
# response is MMI.sba
set.seed(33)  # set seed to get repeatable model              
...
# select cols of interest
dat_MMI.sba <- data_lag1[,c(13,15:(ncol(data_lag1)-1))]
names(dat_MMI.sba) 
...
ggplot(data=gbm1_ri_top, aes(x=varnames,y=RI)) +
  geom_bar(position="dodge",stat="identity") +
  coord_flip() +
  labs(title=paste0("Lag 1 Year: Top ",gbm1_topn," vars for MMI Soft Bodied Algae"))

ggsave(filename = "figs/lag1/lag1_brt_MMI_sba_top_RI_barplot.png",width = 7, height = 7, units = "in", dpi = 300)

###################################################
# MMI.d Lag 1 Year BRT --------------------------------------------------------
names(data_lag1)
# this matches algae from same year against flow from same year
# response is MMI.d
set.seed(33)  # set seed to get repeatable model              
...
# select cols of interest
dat_MMI.d <- data_lag1[,c(14,15:(ncol(data_lag1)-1))]
names(dat_MMI.d) 
...
ggplot(data=gbm1_ri_top, aes(x=varnames,y=RI)) +
  geom_bar(position="dodge",stat="identity") +
  coord_flip() +
  labs(title=paste0("Lag 1 Year: Top ",gbm1_topn," vars for MMI Diatoms"))

ggsave(filename = "figs/lag1/lag1_brt_MMI_d_top_RI_barplot.png",width = 7, height = 7, units = "in", dpi = 300)
