...
(gbm_file <- paste0("models/05e_gbm_final_",tolower(as_name(algaeVar)),"_", tolower(hydroDat), "_", modname, "_hypergrid"))
...
(gbm_best_file <- paste0("models/05e_gbm_final_",tolower(as_name(algaeVar)),"_", tolower(hydroDat), "_", modname, "_", "model_output.txt"))
...
# save to RDS
write_rds(x = get(fileToSave), path = paste0("models/05e_",fileToSave, "_model.rds"), compress = "gz")

# Save all the datasets used in the model:
save(list = ls(pattern="data_"), file = tolower(paste0("models/05e_",fileToSave,"_model_data.rda")))
