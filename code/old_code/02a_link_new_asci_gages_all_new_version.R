...
names(algae)
algae_stations_distinct <- algae[,c(3,6:7)]
algae_stations_distinct<- algae_stations_distinct[!duplicated(algae_stations_distinct$StationCode), ]
dim(algae_stations_distinct) ## 1864
head(algae_stations_distinct)
# sum(is.na(algae_stations_distinct$Latitude))
...
sel_algae_gages
# number of unique?
length(unique(factor(sel_algae_gages$HUC_12))) # h12=167
length(unique(sel_algae_gages$ID)) # gages=209
length(unique(sel_algae_gages$StationCode)) # algae Stations=471
...
sel_algae_gages_asci <- left_join(sel_algae_gages, st_drop_geometry(algae_stations_distinct), by="StationCode") %>%
  # filter(!is.na(asci)) %>%
...
save(gages_not_selected, file="output_data/02a_gages_not_selected.RData")

# get the gages  selected
gages_selected <- gages_all_filt %>% 
  filter(ID %in% sel_gages_algae$ID)

save(gages_selected, file="output_data/02a_gages_selected.RData")

table(sel_gages_algae$CEFF_type) # ALT=151  REF=58
table(gages_not_selected$CEFF_type) # ALT=174  REF=64

# get  algae NOT selected with ASCI
algae_not_selected <- algae %>% 
  # filter(!is.na(asci)) %>% 
  filter(!StationCode %in% sel_algae_gages$StationCode) %>% 
  distinct(StationCode, .keep_all=TRUE)

save(algae_not_selected, file="output_data/02a_algae_sites_not_selected.RData")
...
# save out
write_rds(sel_h12_algae, path="output_data/02a_selected_h12_all_gages.rds")
write_rds(sel_gages_algae, path="output_data/02a_selected_usgs_h12_all_gages.rds")
write_rds(sel_algae_gages_asci, path="output_data/02a_selected_algae_h12_all_gages_asci.rds")
write_rds(sel_algae_gages, path="output_data/02a_selected_algae_h12_all_gages.rds")
...
# use nhdtools to get comids NOT WORKING!!!!!
