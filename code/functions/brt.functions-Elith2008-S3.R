...
function (data,                             # the input dataframe
  ...
  n.minobsinnode = 2,                        ### ANNA added this parameter; appears in 4 lines below when fitting gbm
...
# set up the initial call to gbm
  ...
  if (is.null(offset)) {
    gbm.call <- paste("gbm(y.subset ~ .,data=x.subset, n.trees = n.trees,
      ...
      n.minobsinnode = n.minobsinnode, # added by Anna
...
function (data,                        # the input dataframe
  ...
  n.minobsinnode = 2                   # ANNA added this option
  ) 
{
#
...
# fit the gbm model 
  ...
  gbm.call <- paste("gbm(y.data ~ .,n.trees = n.trees, data=x.data, verbose = F, interaction.depth = tree.complexity, 
    ...
    var.monotone = var.monotone, bag.fraction = bag.fraction, keep.data = keep.data, n.minobsinnode = n.minobsinnode)", sep="")  # ANNA added this here
